"use strict";
let rain_screen = document.getElementById("rain_screen");
const rain_context = rain_screen.getContext("2d");
function draw_raindrop(context, center, height) {
    const width = 1;
    context.beginPath();
    context.moveTo(center[0], center[1]);
    context.quadraticCurveTo(center[0] + width / 1, center[1] + height, center[0] - width / 1, center[1] + height);
    context.fillStyle = "blue";
    context.fill();
}
function update_raindrop(coords, velocity, time) {
    coords[1] = Math.floor(coords[1] + velocity * time);
}
function generateRandomXCoordinate(canvasWidth) {
    return Math.floor(Math.random() * canvasWidth);
}
function simulateRain() {
    const canvasWidth = rain_screen.width;
    let startCoords = [generateRandomXCoordinate(canvasWidth), 0];
...
let start_rain_button = document.getElementById("start_rain_button");
